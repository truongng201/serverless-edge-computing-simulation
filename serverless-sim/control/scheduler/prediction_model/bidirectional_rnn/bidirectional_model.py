#!/usr/bin/env python3
"""
Bidirectional RNN Models for Trajectory Prediction
Includes Bidirectional LSTM and Bidirectional GRU variants
"""
...
class BidirectionalRNNTrajectoryModel:
    """Bidirectional RNN model for trajectory prediction"""
    
    def __init__(self, config: Dict = None):
        """
        Initialize Bidirectional RNN model with configuration
        ...
        self.config = {
            ...
            'rnn_units': [64, 32],  # Bidirectional RNN layers
            ...
            'patience': 15,
            'merge_mode': 'concat',  # How to combine forward and backward: 'concat', 'sum', 'ave', 'mul'
        }
    ...
    def build_model(self) -> keras.Model:
        """Build Bidirectional RNN model architecture"""
        # Input layer
        inputs = layers.Input(
        ...
        for i, units in enumerate(self.config['rnn_units']):
            ...
            # Wrap in Bidirectional layer
            x = layers.Bidirectional(
                rnn_layer,
                merge_mode=self.config['merge_mode'],
                name=f'bidirectional_{self.config["rnn_type"].lower()}_{i+1}'
            )(x)
        ...
        model = keras.Model(
            inputs=inputs,
            outputs=outputs,
            name=f'Bidirectional_{self.config["rnn_type"]}_Trajectory_Model'
        )
    ...
    def train(self, X_train: np.ndarray, y_train: np.ndarray, 
        ...
        """
        print(f"[TRAINING] Training Bidirectional {self.config['rnn_type']} model...")
        print(f"[DATA] Training data shape: {X_train.shape}")
        print(f"[TARGET] Target data shape: {y_train.shape}")
        print(f"[PROCESS] Merge mode: {self.config['merge_mode']}")
        
        # Build model
        self.model = self.build_model()
        
        # Print model summary
        print(f"\n[ARCH] Model Architecture (Bidirectional {self.config['rnn_type']}):")
        self.model.summary()
    ...
    def evaluate(self, X_test: np.ndarray, y_test: np.ndarray, 
        ...
        print(f"[DATA] Evaluating Bidirectional {self.config['rnn_type']} model...")
    ...
    def plot_training_history(self, save_path: str = None):
        ...
        fig.suptitle(f'Bidirectional {self.config["rnn_type"]} Training History (Merge: {self.config["merge_mode"]})', 
                     fontsize=16)
