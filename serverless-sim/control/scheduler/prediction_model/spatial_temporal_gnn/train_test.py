#!/usr/bin/env python3
"""
Training script for Spatial-Temporal GNN trajectory prediction models
"""
...
def train_st_gnn_model(config, model_name):
    ...
    # Create model
    model = SpatialTemporalGNNModel(config)
...
def main():
    ...
    model_configs = [
        ...
        {
            **base_config,
            'num_spatial_nodes': 16,  # 4x4 grid
            'use_temporal_attention': False
        },
        # Medium ST-GNN with Attention
        {
            **base_config,
            'num_spatial_nodes': 16,  # 4x4 grid
        ...
        {
            **base_config,
            'num_spatial_nodes': 25,  # 5x5 grid
            'use_temporal_attention': False
        },
        # Large ST-GNN with Attention
        {
            **base_config,
            'num_spatial_nodes': 25,  # 5x5 grid
            'use_temporal_attention': True
    ...
    model_names = [
        ...
        "ST-GNN Medium (16 nodes)",
        "ST-GNN Medium + Attention (16 nodes)",
    ...
    try:
        ...
        # Compare results
        print("\n" + "=" * 90)
        print("[SPATIAL] SPATIAL-TEMPORAL GNN MODELS COMPARISON")
        print("=" * 90)
        ...
        with open(results_path, 'w') as f:
            f.write("Spatial-Temporal GNN Models - Comprehensive Comparison\n")
            f.write("=" * 60 + "\n\n")
        ...
        print("[SUCCESS] All ST-GNN model training completed successfully!")
...
def create_comparison_plot(all_results):
    """Create comparison plot for all ST-GNN models"""
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(2, 2, figsize=(16, 12))
    
    model_names = [r['model_name'] for r in all_results]
    short_names = [name.replace('ST-GNN ', '').replace(' + Attention', '+Att') for name in model_names]
    ...
    # Attention effect comparison
    attention_models = [r for r in all_results if 'Attention' in r['model_name']]
    non_attention_models = [r for r in all_results if 'Attention' not in r['model_name']]
    
    if attention_models and non_attention_models:
        att_errors = [r['evaluation_results']['mean_error_meters'] for r in attention_models]
        non_att_errors = [r['evaluation_results']['mean_error_meters'] for r in non_attention_models]
        
        att_names = [r['model_name'].replace(' + Attention', '') for r in attention_models]
        non_att_names = [r['model_name'] for r in non_attention_models]
        
        x = np.arange(len(att_names))
        width = 0.35
        
        if len(att_errors) == len(non_att_errors):
            ax3.bar(x - width/2, non_att_errors, width, label='Without Attention', color='orange', alpha=0.7)
            ax3.bar(x + width/2, att_errors, width, label='With Attention', color='purple', alpha=0.7)
            ax3.set_title('Attention Mechanism Effect')
            ax3.set_xlabel('Model Size')
            ax3.set_ylabel('Mean Error (meters)')
            ax3.set_xticks(x)
            ax3.set_xticklabels(['Medium', 'Large'])
            ax3.legend()
        else:
            # Fallback: show all pixel accuracies
            pixel_accs = [r['training_results']['final_pixel_accuracy'] for r in all_results]
            ax3.bar(range(len(short_names)), pixel_accs, color='lightgreen', alpha=0.7)
            ax3.set_title('Pixel Accuracy Comparison')
    ...
    # Save plot
    plot_path = os.path.join(os.path.dirname(__file__), 'st_gnn_models_comparison.png')
