...
class CentralNodeAPIController:
    ...
    def _get_or_create_container(self, function_name: str, image: str) -> Tuple[str, str]:
        # If per-user sticky functions are enabled, prefer reusing the container that already
        # belongs to this function_name (if warm and not expired). Otherwise, reuse any warm
        ...
        if Config.STICKY_FUNCTION_PER_USER:
            for container in warm_containers:
                ...
                if container.started_at and now - container.started_at > Config.DEFAULT_MAX_WARM_TIME:
                    ...
                    # Remove so the name can be reused for a fresh cold start
