...
export const useEventHandlers = (state, actions) => {
  ...
  const {canvasRef, editMode} = useSimulationStore();
  ...
  // Zoom functions
  const zoomIn = useCallback(() => setZoomLevel((prev) => Math.min(prev * 1.2, 5)), [setZoomLevel]);
  const zoomOut = useCallback(() => setZoomLevel((prev) => Math.max(prev / 1.2, 0.2)), [setZoomLevel]);
